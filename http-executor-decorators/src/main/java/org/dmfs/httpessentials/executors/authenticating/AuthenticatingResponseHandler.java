...
/**
 * A response handler for "401 Unauthorized" responses. When called, it decorates the request with a new {@link AuthState} and tries to execute it again.
 *
 * @author Marten Gajda
 */
final class AuthenticatingResponseHandler<T> implements HttpResponseHandler<T>
{
    ...
    AuthenticatingResponseHandler(HttpRequestExecutor executor, AuthState authState, URI uri, HttpRequest<T> request)
    {
        mExecutor = executor;
    ...
    {
        // start over with the new AuthState state
        return mExecutor.execute(
                response.requestUri(),
                new Authenticated<>(mExecutor, mAuthState, mUri, mRequest));
    }
}
