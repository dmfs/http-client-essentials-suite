...
/**
 * An exception that's thrown when a response status code was unexpected. <p> There are three major subclasses: {@link
 * RedirectionException} for <code>3xx</code> response status codes, {@link ClientErrorException} for <code>4xx</code>
 * response status codes and {@link ServerErrorException} for <code>5xx</code> response status codes, some of them
 * having subclasses themselves. Implementations should always pick the most appropriate error class to allow for proper
